...
def check_sys_and_update_dll():
    ...
    if currentsystem == 'Windows':
        # Đường dẫn tuyệt đối chuẩn tới thư mục DLL của MVS trên Windows
        if bit_info == "64":
            dll_dir = r"C:\Program Files (x86)\Common Files\MVS\Runtime\Win64_x64"
        else:
            dll_dir = r"C:\Program Files (x86)\Common Files\MVS\Runtime\Win32_i86"

        MvCamCtrldllPath = os.path.join(dll_dir, "MvCameraControl.dll")

        # Nạp thư mục DLL vào hệ thống (bắt buộc cho Windows với Python 3.8+)
        if hasattr(os, 'add_dll_directory') and os.path.exists(dll_dir):
            os.add_dll_directory(dll_dir)
        os.environ['PATH'] = dll_dir + ';' + os.environ.get('PATH', '')

        # Load file DLL trực tiếp
        if os.path.exists(MvCamCtrldllPath):
            MvCamCtrldll = WinDLL(MvCamCtrldllPath)
        else:
            # Fallback về tên mặc định nếu người dùng tự copy file DLL về cạnh script
            MvCamCtrldll = WinDLL("MvCameraControl.dll")
    else:
        architecture = platform.machine()
        run_env = os.getenv('MVCAM_COMMON_RUNENV', '')
        if architecture == 'aarch64':
            MvCamCtrldllPath = run_env + "/aarch64/libMvCameraControl.so"
        elif architecture == 'x86_64':
            if bit_info == "32":
                MvCamCtrldllPath = run_env + "/32/libMvCameraControl.so"
            else: 
                MvCamCtrldllPath = run_env + "/64/libMvCameraControl.so"
        elif architecture in ['arm-none', 'armhf', 'armv6l', 'armv7l']:
            MvCamCtrldllPath = run_env + "/armhf/libMvCameraControl.so"
